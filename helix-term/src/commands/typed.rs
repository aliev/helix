...
fn summarize_undo_revision(revision: RevisionInfo<'_>, current_text: &Rope, root_text: &Rope) -> String {
    if revision.id == 0 {
...
impl GitConflictPopup {
    fn new(
        ...
        let ours_label = conflict_side_label(text, conflict.start_line, CONFLICT_START_MARKER, "current branch");
        let theirs_label =
        ...
        match event {
            compositor::Event::Key(key) if key.modifiers.is_empty() => match key.code {
                ...
                KeyCode::Char('l') => compositor::EventResult::Consumed(Some(Box::new(
                    |_compositor, cx| {
                        if let Err(err) =
                            copy_git_line_blame_permalink(cx, Args::default(), PromptEvent::Validate)
                        {
                            cx.editor.set_error(err.to_string());
                        }
                    },
...
fn current_git_blame_info(editor: &Editor) -> anyhow::Result<GitLineBlameInfo> {
    ...
    parse_git_blame_output(
        &permalink.repo_root,
        &remote_to_web_url(&permalink.remote_url)?,
        &relative_path,
        line_number,
...
fn copy_git_line_blame_remote(
    ...
    cx.editor.set_status("Copied blamed commit URL to system clipboard");
    Ok(())
}
...
pub const TYPABLE_COMMAND_LIST: &[TypableCommand] = &[
    ...
    TypableCommand {
        name: "undo-list",
        aliases: &[],
        doc: "Open a picker with undo history and preview each revision.",
        fun: undo_list,
        completer: CommandCompleter::none(),
    ...
    TypableCommand {
        name: "git-line-blame",
        aliases: &[],
        doc: "Show git blame information for the current line in a popup.",
        fun: preview_git_line_blame,
        completer: CommandCompleter::none(),
        signature: Signature {
            positionals: (0, Some(0)),
            ..Signature::DEFAULT
